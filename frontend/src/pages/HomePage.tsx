...
function findNearestStation(
  ...
  return (
    stations
      ...
      .map((s) => ({
        station: s,
        distance: distanceKm(centroid.lat, centroid.lng, Number(s.lat), Number(s.lon)),
      }))
...
export default function HomePage({
  ...
  const { pdfSaving, pdfError, setPdfError, pdfSuccess, resetPdfSuccess, handleGeneratePdf } = useGeneratePdf();
  ...
  const {
    ...
    elev1, setElev1,
    elev2, setElev2,
    heightDifference, setHeightDifference,
    length, setLength,
  ...
  const ivfPanelProps = { ivfData, ivfLoading, ivfError, selectedStation };
  ...
  function PanelToggle({ className = "" }: { className?: string }) {
    return (
      <div className={`flex overflow-hidden rounded-xl border border-slate-200 bg-white/95 shadow-lg backdrop-blur dark:border-slate-700 dark:bg-slate-900/95 ${className}`}>
        <button type="button" onClick={handleShowMap} className={`flex items-center justify-center px-4 py-2 transition ${rightPanelView === "map" ? "bg-[#213F53] text-white" : "text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800"}`} aria-label="Kartvisning" title="Kart">
          <Map size={18} />
        </button>
        <button type="button" onClick={handleShowIvf} className={`flex items-center justify-center px-4 py-2 transition ${rightPanelView === "ivf" ? "bg-[#213F53] text-white" : "text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800"}`} aria-label="IVF-tabell" title="IVF-tabell">
          <Table2 size={18} />
        </button>
      </div>
    );
  ...
  function MobileTabButton({ tab, icon, label }: { tab: MobileTab; icon: React.ReactNode; label: string }) {
    const active = mobileTab === tab;
    return (
      <button type="button" onClick={() => setMobileTab(tab)} className={`flex flex-1 items-center justify-center gap-1.5 py-2.5 text-xs font-medium transition ${active ? "border-b-2 border-[#213F53] text-[#213F53] dark:border-sky-400 dark:text-sky-400" : "text-slate-500 dark:text-slate-400"}`}>
        {icon}
        {label}
      </button>
    );
      ...
      <main className="h-[calc(100dvh-4rem)] min-h-0 overflow-hidden">
        ...
        <div className="flex h-full min-h-0 flex-col xl:hidden">
          <div className="flex shrink-0 border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
            ...
            <MobileTabButton tab="sidebar" icon={<SlidersHorizontal size={15} />} label="Innstillinger" />
          </div>
